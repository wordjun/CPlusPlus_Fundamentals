...
char* makeStrAdr1(int len) {
	//문자열 저장을 위한 배열을 힙 영역에 할당하고 있음
	char* str = (char*)malloc(sizeof(char) * len);//char형 길이 * 길이(len)만큼 할당
	return str;
}
//C++스타일의 동적할당
...
int main(void) {
	//할당할 대상의 정보를 무조건 바이트크기단위로 전달해야 함
	//반환형이 void형 포인터이기 때문에 적절한 형변환이 필요하다(char*)
